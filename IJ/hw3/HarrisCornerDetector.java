...
public class HarrisCornerDetector {
	...
	List<Corner> findCorners(){
		makeDerivatives();
		makeCrf();	//corner response function (CRF)
		corners = collectCorners(params.border);
//		IJ.showMessage("Corners before cleanup: " + corners.size());
		if (params.doCleanUp) {
			corners = cleanupCorners(corners);
//			IJ.showMessage("Corners after cleanup: " + corners.size());
		}
		return corners;
	}
...
//		FloatProcessor Iy = (FloatProcessor) ipOrig.convertToFloat();
		...
		applySeperableKernel_Horiz(Ix, pfilt);
		applySeperableKernel_Horiz(Ix, dfilt);
		
		applySeperableKernel_Vert(Iy, pfilt);
		applySeperableKernel_Vert(Iy, dfilt);

		A = (FloatProcessor) Ix.duplicate();
		A.sqr();
		applySeperableKernel(A, bfilt);
		
		B = (FloatProcessor) Iy.duplicate();
		B.sqr();
		applySeperableKernel(B, bfilt);
		
		C = ((FloatProcessor) Ix.duplicate());
		C.copyBits(Iy, 0, 0, Blitter.MULTIPLY);
		applySeperableKernel(C, bfilt);
	}
